...
随机变量独立同分布，且具有相同的数学期望和方差：$E(X_k) = mu, D(X_k) = sigma^2$，则随机变量之和的标准化变量：$Y_n = (sum^n_(k=1) x_i - n mu) / (sqrt(n) sigma)$的分布函数对于任意$x$满足$limits(lim)_(n->oo) P{(sum^n_(k=1) x_i - n mu) / (sqrt(n) sigma) <= x } = Phi(x)$，即收敛于标准正态分布的分布函数。
